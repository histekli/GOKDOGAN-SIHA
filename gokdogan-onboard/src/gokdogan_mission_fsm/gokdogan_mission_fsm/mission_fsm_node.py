...
_T_NAMES = {T_IDLE: "-", T_WAIT_CONNECT: "WAIT_CONNECT", T_SET_MODE: "SET_MODE",
            T_ARM: "ARM", T_TAKEOFF_CMD: "TAKEOFF_CMD", T_CLIMB: "CLIMB",
            T_DONE: "DONE", T_FAILED: "FAILED"}
...
class MissionFsmNode(LifecycleNode):
    ...
    def on_configure(self, state) -> TransitionCallbackReturn:
        ...
        self._pub_mode = self.create_lifecycle_publisher(
            MissionMode, "/mission/mode", qos.mission_mode())

        self.create_subscription(State, "/mavros/state", self._on_state,
                                 10, callback_group=cbg_io)
        self.create_subscription(Float64, "/mavros/global_position/rel_alt",
                                 self._on_alt, qos.sensor_stream(), callback_group=cbg_io)
        ...
        self._helper_node = rclpy.create_node(
            self.get_name() + "_mav", context=self._helper_ctx)
        ...
        self._cli["stream_rate"] = self._helper_node.create_client(
    ...
    def _control_step(self):
        ...
        if ph == T_WAIT_CONNECT:
            ...
            if self._rel_alt is not None and self._rel_alt >= 0.95 * alt_target:
                self.get_logger().info(
                    f"Kalkış TAMAM: rel_alt={self._rel_alt:.2f} m → CRUISE")
            ...
            if now - self._last_fire.get("climb_log", 0.0) > 3.0:
                ...
                self.get_logger().info(
                    f"climb: rel_alt={self._rel_alt} armed={self._mav.armed} mode={self._mav.mode}")
            # Tırmanış başlamadıysa arm+takeoff'u yeniden dene (disarm olmuşsa yeniden arm eder)
            if (self._rel_alt is None or self._rel_alt < 1.0) and now - self._phase_t0 > 6.0 \
